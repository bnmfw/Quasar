...
class LetFinder:
    ...
    def __verificar_validacao(self, circuito, vdd: float, let: LET) -> tuple:
        ...
        if (inclinacao_saida == "rise" and tensao_pico_saida > vdd * 0.51) or\
            ...
            if relatorio: print("Analise invalida - Tensoes improprias\n")
            return (False, 1)
        ...
        if (inclinacao_saida == "rise" and tensao_pico_saida < vdd * 0.50) or\
            ...
            if relatorio: print("Analise invalida - Pulso sem efeito\n")
            return (False, 2)

        return (True, 2)

    ##### ENCONTRA A CORRENTE MAXIMA PARA UM LET #####
    def __encontrar_corrente_maxima(self, circuito, let:LET) -> float:
        ...
        for _ in range(10):
            ...
            if (inclinacao_saida == "rise" and tensao_pico_saida > circuito.vdd/2) or\
                ...
                if relatorio: print(f"PULSO MAXIMO ENCONTRADO ({self.__limite_sup})")
    ...
    def __encontrar_corrente_minima(self, circuito, let: LET) -> float:
        ...
        for _ in range(self.__limite_sim):
            ...
            if diferenca_largura and -precisao_largura < diferenca_largura < precisao_largura:
                if relatorio: print(f"PULSO MINIMO ENCONTRADO - PRECISAO SATISFEITA ({corrente})")
                return corrente


            if abs(csup - cinf) < 1:
                if relatorio: print(f"PULSO MINIMO ENCONTRADO - DIFERENCA DE BORDAS PEQUENA ({corrente})")
                return corrente
        ...
        if relatorio: print(f"PULSO MINIMO NAO ENCONTRADO - LIMITE DE SIMULACOES ATINGIDO")
        return None
    ...
    def definir_corrente(self, circuito, let: LET, validacao: list, safe: bool = False, delay: bool = False) -> int:
        ...
        with HSRunner.Inputs(vdd, entradas):
            ...
            if not safe:
                analise_valida, simulacoes = self.__verificar_validacao(circuito, vdd, let)
            ...
            cinf: float  = 0
            cinf: float = 0 if not delay else self.__encontrar_corrente_minima(circuito, let)
            corrente: float = cinf
            sup_flag: bool = False
            ...
            for i in range(self.__limite_sim):
                ...
                if (1 - precisao) * vdd / 2 < tensao_pico < (1 + precisao) * vdd / 2:
                    if relatorio: print("LET ENCONTRADO - PRECISAO SATISFEITA\n")
                ...
                ##### CONVERGENCIA DA CORRENTE ####
                    ...
                    if 1 < corrente < limite_sup - 1:
                        if relatorio: print("LET ENCONTRADO - CONVERGENCIA\n")
                        let.corrente = corrente
                        let.append(validacao)
                        return simulacoes
                    #### CONVERGENCIA PARA 0 ####
                    elif corrente <= 1:
                        if relatorio: print("LET NAO ENCONTRADO - DIVERGENCIA NEGATIVA\n")
                        let.corrente = None
                        return simulacoes
                    #### CONVERGENCIA PARA LIMITE SUPERIOR ####
                    else:
                        if relatorio: print("LIMITE SUPERIOR AUMENTADO - DIVERGENCIA POSITIVA\n")
                        csup += 100
                        limite_sup += 100
                        sup_flag = True
            ...
            if 1 < corrente <self.__limite_sup - 1:
                if relatorio: print("LET ENCONTRADO - LIMITE DE SIMULACOES ATINGIDO\n")
                let.corrente = corrente
                let.append(validacao)
            
            ### DIVERGENCIA ###
            else:
                if relatorio: print("LET NAO ENCONTRADO - LIMITE DE SIMULACOES ATINGIDO\n")
                let.corrente = 99999 if sup_flag else 11111
            return simulacoes

LetManager = LetFinder()