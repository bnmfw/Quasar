...
class PSGUI:
    class SimpleInputScreen:
        def __init__(self, label: str, tipo: type, tooltip: str = None):
            ...
            self.tipo = tipo
            layout = [
                [sg.Text(label), sg.InputText(tooltip=tooltip)],
                [self.warning_text],
                [sg.Button("Ok")],
            ]
    ...
    class SimpleProgressBar:
        def __init__(self, label: str) -> None:
            self.max = 1000
            self.progresso = sg.ProgressBar(
                self.max, orientation="h", expand_x=True, size=(20, 20)
            )
            layout = [
                [sg.Text(label)],
                [self.progresso],
            ...
            self.window = sg.Window(
                "Progress Bar", layout, size=(715, 100), finalize=True
            )
    ...
    def tela_inicial(self):
        ...
        def get_circuits():
            ...
            available_circuits = [
                circ for circ in os.listdir("circuitos") if not circ in not_circuits
            ]
        ...
        start_layout = [
            [sg.Text("Quasar", justification="center")],
            [
                sg.Text("Escolha um Circuito"),
                sg.Combo(get_circuits(), enable_events=True),
            ],
            [file_exists],
            [
                sg.Text("Vdd (V)"),
                sg.InputText(tooltip=" Use virgula como separador decimal "),
            ],
            [warning_text],
    ...
    def tela_cadastro(self, circuito_nome):
        ...
        self.progresso = sg.ProgressBar(
            self.progress_max, orientation="h", expand_x=True, size=(10, 10)
        )
        ...
        nodos = list(HSRunner.get_nodes(circuito_nome))
        nodos.sort()
        inputs["nodos"] = nodos
        ...
        while True:
            ...
            if event == "Ok":
                ...
                inputs["saidas"] = [
                    nodo for id, nodo in nodo_dict.items() if values[id] is True
                ]
    ...
    def tela_principal(self, circuito):
        inputs = {"acao": None}
        layout = [
            [sg.Text(f"Circuito {circuito.nome} operando em {sim_config.vdd}V")],
    ...
    def tela_mc(self, circuito):
        ...
        layout_bc = [
            [sg.Text("Existe uma simulacao Monte Carlo em andamento")],
            [
                sg.Text(
                    "Se deseja iniciar uma nova simulação Insira o numero de simulacoes"
                )
            ],
        ...
        layout_nbc = [
            [sg.Text("Quantidade de analises a analisar:")],
            [sg.InputText()],
            [warning_text],
            [iniciar],
            [self.progresso],
