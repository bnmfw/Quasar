    ...
    def __init__(self) -> None:
        ...
        while True:
            ...
            if current_screen == "start":
                ...
                config_log = sim_config.load(
                    path.join("project", "circuits", f"{circ_nome}")
                )
                if cadastro:
                    self.circuito = Circuito(circ_nome).from_json()
                    self.quasarAPI.set_circuit(self.circuito)
                elif not config_log:
                    _, inputs = self.ui.tela_config_sim(
                        sim_config.vdd,
            ...
            # TELA DE CADASTRO
                ...
                self.circuito = Circuito(circ_nome).from_nodes(
                    inputs["entradas"], inputs["saidas"]
                )
                print(self.circuito.nodes)
                self.quasarAPI.set_circuit(self.circuito)
                with HSRunner.Vdd(sim_config.vdd):
                    self.quasarAPI.determine_LETs(
                        progress_report=self.ui.progress, log_circuit=True
                    )
            ...
            # TELA PRINCIPAL
                ...
                if inputs["acao"] == "atualizar":
                    ...
                    CManager.write_full_csv(
                        self.circuito, self.circuito.path_to_circuits
                    )
            ...
            # SINGLE LET SCREEN
                ...
                with HSRunner.Vdd(sim_config.vdd):
                    self.quasarAPI.find_single_let(
                        inputs["node"],
                        inputs["output"],
                        inputs["input"],
                        *inputs["pulses"],
            ...
            # SIMULATION CONFIGURATION SCREEN
                ...
                current_screen, inputs = self.ui.tela_config_sim(
                    sim_config.vdd,
                    sim_config.fault_model.colect_time,
                    sim_config.fault_model.track_estab,
                    sim_config.transistor_model.charge_collection_depth_nano,
