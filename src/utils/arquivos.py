...
class CSVManager:
    ...
    def write_full_csv(circuito, path_to_circuits: str):
        with open(
            path.join(path_to_circuits, circuito.name, f"{circuito.name}.csv"), "w"
        ) as sets:
...
class JsonManager:
    ...
    def codify(self, circuito, path_to_circuits=path.join("project", "circuits")):
        ...
        dicionario_de_nodos = (
            {}
        )  # criacao do dicionario que tera o dicionario de todos os nodos
        lista_de_nodos = []
        for nodo in circuito.nodes:
        ...
        if not path.exists(
            path.join(path_to_circuits, circuito.name, f"{circuito.name}.json")
        ):
            json.dump(
                circuito_codificado,
                open(
    ...
    def decodify(self, circuito, path_to_circuits=path.join("project", "circuits")):
        circuito_codificado: dict = json.load(
            open(
                path.join(path_to_circuits, circuito.name, f"{circuito.name}.json"), "r"
            )
        )
