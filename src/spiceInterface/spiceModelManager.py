...
class SpiceModel:
    ...
    def append(self, atribute_line: list) -> None:
        ...
        """
        self.model_atributes.append(list(map(lambda e: {"name": e[0], "nominal": float(e[1]), "value": float(e[1])}, atribute_line)))
    ...
    def compiled(self) -> str:
        ...
        for atribute_line in self.model_atributes:
            model_string += "+" + tab.join(f'{att["name"].ljust(8)}= {str(att["value"]).ljust(10)}' for att in atribute_line) + "\n"
        return model_string
    ...
    def parseModelFile(self, filename: str) -> None:
        ...
        with open(filename, "r") as model_file:
            for i, line in enumerate(model_file):
                line = line.strip()
                if not i or not len(line) or line[0] == "*": continue
                if line[0] == ".":
                    tokens = [t.lower() for t in line.split()]
                    if current_model is not None and current_model.name in self.models_name_used: 
                        self.models[current_model.name] = current_model
                    current_model = SpiceModel(tokens[1], tokens[2], " ".join(tokens[3:]))
                    {"name": tokens[1], "type": tokens[2], "extra": " ".join(tokens[3:]), "attributes": []}
                if line[0] == "+":
                    line = line[1:]
