...
def Ler_pulso(direcaoPulsoSaida,offset):
	...
	#print(max_tensao, min_tensao)
        ...
        if direcaoPulsoSaida == "up": tensao_pico = max_tensao
        elif direcaoPulsoSaida == "down": tensao_pico = min_tensao
...
def Definir_fontes(fontes,validacao,vdd,entradas):
	with open(fontes, "w") as sinais:
                ...
                for i in range(len(validacao)):
                        ...
                        if validacao[i] == 1: sinais.write(str(vdd)+"\n")
                        elif validacao[i] == 0: sinais.write("0.0\n")
                        else: print("ERRO SINAL NAO BINARIO RECEBIDO: ",validacao[i])
...
def Verificar_validacao(circuito, arqvRadiacao, nodo, direcaoPulsoNodo, saida, direcaoPulsoSaida, vdd):
	...
	#print(tensaoPicoNodo, tensaoPicoSaida)

	if direcaoPulsoSaida == "up" and tensaoPicoSaida > vdd*0.8:
		return False
	elif direcaoPulsoSaida == "down" and tensaoPicoSaida < vdd*0.2:
		return False
	elif direcaoPulsoNodo == "up" and tensaoPicoNodo > vdd*0.8:
		return False
	elif direcaoPulsoNodo == "down" and tensaoPicoNodo < vdd*0.2:
		return False
	else:
		return True
	

def Corrente(circuito, entradas, direcaoPulsoNodo, direcaoPulsoSaida, nodo, saida, validacao):
	radiacao = "SETs.txt"
	fontes = "fontes.txt"
