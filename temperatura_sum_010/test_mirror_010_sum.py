...
def set_measures(type_measure):
	...
	if type_measure == 0:
		x = '.meas tran minout min V(nsum) from=1p to=8n\n.meas tran maxout max V(sum) from=1n to=8n'
	else:
		x = '.meas tran minout min V(sum) from=1p to=8n\n.meas tran maxout max V(nsum) from=1p to=8n'
...
while (input_vector_counter != 8):
	...
	while (table_node_counter != len(table_nodes)):
		...
		current_pulse = 0			#zera o pulso de corrente para o novo no
		...
		while((current_pulse <= 120) and (set_occurence == False)):
			...
			completed = subprocess.run(['hspice ADDER_MIRROR_COUT_1.cir | grep "minout\|maxout"'], shell=True, stdout=subprocess.PIPE,)
...
with open('min_max_010_sum.txt', 'w') as filehandle:
	for listitem in table_min_max:
		filehandle.write('%s\n' % listitem)


with open('results_010_sum.txt', 'w') as filehandle:
